...
public class PoguesSearch {
	...
	public List<ResponseSearchItem> search(
			...
			PoguesQuery query, @Context UriInfo info) throws Exception {
		try {
	...
	@Path("items/{label}")
	@Produces(MediaType.APPLICATION_JSON)
	@ApiOperation(value = "Get items from Solr", notes = "Get items from Solr pertinent with the label")
	public List<SolrResult> getSearchEchos(@PathParam(value = "label") String label) throws Exception {
		try {
			return searchService.getSolrResults(label);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			throw e;
		}
